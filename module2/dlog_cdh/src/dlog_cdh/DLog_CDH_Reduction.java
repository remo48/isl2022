...
public class DLog_CDH_Reduction extends A_DLog_CDH_Reduction<IGroupElement, BigInteger> {
    ...
    public BigInteger run(I_DLog_Challenger<IGroupElement> challenger) {
        ...
        if (this.generator.power(BigInteger.ZERO).equals(challenge.x)) {
        ...
        for (int i = 0; i < primes.length; i++) {
            int prime = primes[i];
            BigInteger n = groupOrder.subtract(BigInteger.ONE).divide(BigInteger.valueOf(prime));
            IGroupElement xi = cdh_power(challenge.x, n);
            IGroupElement zi = challenge.generator.power(multiplicativeGenerator);

            for (int j = 0; j < prime; j++) {
                if (xi.equals(cdh_power(zi, n.multiply(BigInteger.valueOf(j))))) {
                    values[i] = j;
                    break;
                }
            }
    ...
    private IGroupElement cdh(IGroupElement x, IGroupElement y) {
        ...
        cdh_challenge = new CDH_Challenge<IGroupElement>(this.generator, x, y);
