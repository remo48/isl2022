...
class Point(object):
    ...
    def double(self):
        ...
        l = ((3*self.x**2 + self.curve.a) * mod_inv(2*self.y, self.curve.p)) % self.curve.p
    ...
    def add(self, other):
        ...
        if isinstance(other, PointInf):
            ...
            return PointInf(self.curve)
        elif self.is_equal(other):
            return self.double()

        l = ((other.y - self.y) * mod_inv(other.x - self.x, self.curve.p)) % self.curve.p
        x = (l**2 - self.x - other.x) % self.curve.p
        y = (l * (self.x - x) - self.y) % self.curve.p
        return Point(self.curve, x, y)
    ...
    def scalar_multiply(self, scalar):
        ...
        scalar_bin = list(bin(scalar))[3:] # binary repr. without most significant bit
        p = self
        root_p = p
        for b in scalar_bin:
            p = p.double()
